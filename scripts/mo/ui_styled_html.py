...
def records_table(records: List) -> str:
    ...
    table_html += (
        '<div class="mo-col mo-col-preview"><span class="mo-text-header">Preview</span></div>'
    )
    ...
    table_html += (
        '<div class="mo-col mo-col-actions"><span class="mo-text-header">Actions</span></div>'
    )
    table_html += "</div>"
    for record in records:
        ...
        table_html += (
            f'<img class="mo-preview-image" src="{preview_url}" '
            f'alt="Preview image"'
        ...
        if record.is_local_file_record():
            ...
            table_html += (
                '<button type="button" class="mo-btn mo-btn-success" '
                f"onclick=\"navigateEditPrefilled('{json_record}')\">Add</button><br>"
            )

            table_html += (
                '<button type="button" class="mo-btn mo-btn-danger" '
            ...
            table_html += (
                '<button type="button" class="mo-btn mo-btn-warning" '
...
def _create_groups(groups: List) -> str:
    ...
    for group in groups:
        groups_html += (
            f'<span class="mo-badge mo-badge-group" onclick="">{html.escape(group)}</span>'
        )
    return groups_html
...
def _details_top(record: Record) -> str:
    ...
    content += (
        f'<img class="mo-details-image" src="{preview_url}" alt="Preview Image" '
        f"onerror=\"this.onerror=null; this.src='{_no_preview_image_url()}';\"/>"
    )
    content += "</div>"
    ...
    if record.positive_prompts or record.negative_prompts:
        ...
        if record.positive_prompts:
            ...
            content += (
                '<span class="mo-text-header mo-text-positive-header">Positive Prompts:</span>'
            )
            content += "</div>"
        ...
        if record.negative_prompts:
            ...
            content += (
                '<span class="mo-text-header mo-text-negative-header">Negative Prompts</span>'
            )
            content += "</div>"
        content += "</div>"
...
def records_cards(records: List) -> str:
    ...
    for record in records:
        ...
        preview_url = get_best_preview_url(record)
        content += (
            f'<img src="{preview_url}" alt="Preview Image" '
            f"onerror=\"this.onerror=null; this.src='{_no_preview_image_url()}';\"/>"
        )
        ...
        content += (
            f'<div class="mo-card-text-left"><span class="mo-badge {_model_type_css_class(record.model_type)}"'
...
def _download_url(record_id, url: str, is_preview: bool) -> str:
    ...
    content = (
        f'<p style="margin-top: 2rem; display: block; overflow-wrap: anywhere;" id="url{preview}-{record_id}">'
        f"[{hint}]: {url}</p>"
    )
    return content
...
def _download_progress_bar(record_id, is_preview: bool) -> str:
    preview = "-preview" if is_preview else ""
    content = (
        '<div class="mo-progress" style="height: 1.2rem; margin-top: 1rem; display: none" '
...
def download_cards(records: List, token) -> str:
    ...
    for record in records:
        ...
        content += (
            f'<div class="mo-downloads-card mo-alert-secondary" id="download-card-{id_}" '
